...
void CompactionIterator::NextFromInput() {
  ...
  while (!valid_ && input_->Valid() && !IsPausingManualCompaction() &&
    ...
    if (need_skip) {
      ...
      #ifdef ON_DCPMM
      if (kvs_enabled && ikey_.type == kTypeValue) KVSFreeValue(value_);
      #endif
      value_.clear();
      valid_ = true;
      ...
      if (input_->Valid() && ParseInternalKey(input_->key(), &next_ikey) &&
        ...
        if (prev_snapshot == 0 ||
          ...
          if (next_ikey.type == kTypeSingleDeletion) {
            ...
            #ifdef ON_DCPMM
              if(kvs_enabled && ikey_.type == kTypeValue){
                KVSFreeValue(input_->value());
              }
            #endif
            input_->Next();
          } else {
      ...
      while (input_->Valid() && ParseInternalKey(input_->key(), &next_ikey) &&
             ...
             (prev_snapshot == 0 ||
              DEFINITELY_NOT_IN_SNAPSHOT(next_ikey.sequence, prev_snapshot))) {
        #ifdef ON_DCPMM
        if (kvs_enabled && next_ikey.type == kTypeValue) KVSFreeValue(input_->value());
        #endif
        input_->Next();
      }
