...
uint64_t max_memtable_entries{0};
std::atomic<uint64_t> scheduled_compactions{0};
...
uint64_t DBImpl::ResetMaxL0FilesStats(uint64_t& max_l0, uint64_t& max_mem, uint64_t& scheduled_compaction) {
  max_l0 = max_l0_files;
  max_l0_files = current_l0_files;
  max_mem = 0;
  max_memtable_entries = 0;
  scheduled_compaction = scheduled_compactions.exchange(0, std::memory_order_relaxed);
  return 0;
}
...
Status DBImpl::GetImpl(const ReadOptions& read_options, const Slice& key,
  ...
  PERF_TIMER_GUARD(total_get_time);
#ifndef NDEBUG
  ...
  bool skip_memtable = read_options.skip_memtable || (read_options.read_tier == kPersistedTier &&
                        has_unpersisted_data_.load(std::memory_order_relaxed));
  bool done = false;
...
#ifdef ON_DCPMM
  if (s.ok() && KVSEnabled()){
    KVSDecodeValueRef(get_impl_options.value->data(), get_impl_options.value->size(), get_impl_options.value->GetSelf());
    if (get_impl_options.value->IsPinned()){
...
std::vector<Status> DBImpl::MultiGet(
  ...
  for (keys_read = 0; keys_read < num_keys; ++keys_read) {
    ...
    bool skip_memtable = read_options.skip_memtable ||
